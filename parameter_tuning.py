...
def n_estimators_tuning():
    ...
    line1 = plt.plot(n_estimators, validation_results, label="Validation AUC")    
...
def max_depth_tuning():
    ...
    for i in max_depth:
        ...
        roc_auc = auc(false_positive_rate, true_positive_rate)
        validation_results.append(roc_auc)
    line1 = plt.plot(max_depth, validation_results, label = "Validation AUC")
    plt.ylabel("AUC Score")
...
def min_sample_split():
    ...
    for i in min_sample_split:
        ...
        roc_auc = auc(false_positive_rate, true_positive_rate)
        validation_results.append(roc_auc)
    line1 = plt.plot(min_sample_split, validation_results, label = "Validation AUC")
    plt.ylabel("AUC Score")
